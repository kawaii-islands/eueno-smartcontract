...
impl<H: Domain, G: Domain> VerifierStackedDrg<H, G> {
    ...
    pub fn satisfies_requirements(
        ...
        (partition_challenges * partitions) as u64 >= requirements.minimum_challenges
    }
    ...
    pub fn generate_public_inputs(
        ...
        for challenge in all_challenges.into_iter() {
            ...
            inputs.extend(generate_inclusion_inputs(&por_params_d, challenge, k)?);
            ...
            for parent in drg_parents.into_iter() {
                inputs.extend(generate_inclusion_inputs(&por_params, parent as usize, k)?);
            }
    ...
    pub fn verify(
        ...
        if !Self::satisfies_requirements(&public_params, requirements, 1) {
            println!("failed to satisfy requirements");
            return Ok(false);
        }
        ...
        if !res {
            println!("failed to verify groth16 proof")
        };
        Ok(res)
    }
}
